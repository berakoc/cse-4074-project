...
const GetHandler = (httpRequestHeader, socket) => {
    ...
    const $socket = socket
    const notImplementedRequestMethods = ['PUT', 'HEAD', 'DELETE', 'POST', 'CONNECT', 'OPTIONS', 'TRACE', 'PATCH']
    ...
    return {
        getRequestValue: (key) => {
            return $requestHeader[key] || createErrorMethod('Key is not found in request header.')
        },
        handle: () => {
            const requestInfo = $requestHeader['Request-Info']
            const requestMethod = requestInfo.substring(0, requestInfo.indexOf(' '))
            const size = (() => {
                const subRequestInfo = requestInfo.substring(requestInfo.indexOf('/') + 1)
            ...
            if (requestMethod === 'GET') {
                ...
                [[code, status], responseBody] = [[501, 'NOT IMPLEMENTED'], '501::Not Implemented']
            } else {
                [[code, status], responseBody] = [[400, 'BAD REQUEST'], '400::Bad Request']
            }
            $socket.write(createResponseHeader(code, status, responseBody.length))
            $socket.write(responseBody)
            $socket.end()
            console.log(`Response sent to the agent[${$requestHeader['User-Agent']}].`)
        }
    }
}
